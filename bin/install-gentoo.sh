...
do
    ...
    mount $device"1" /temp
    if [ $? = 0 ]; then
		if [ -e /temp/bzImage ] && [ -e /temp/initramfs.cpio.gz ] && [ -e /temp/ps4gentoo.tar.gz ]; then
...
cp /temp/ps4gentoo.tar.gz /backup
if [ $? -ne  0 ]; then
...
mke2fs-new -t ext4 -F -L gentoo -O ^has_journal $device"2" 
...
tar -xvpzf /backup/ps4gentoo.tar.gz -C /newroot --numeric-owner
